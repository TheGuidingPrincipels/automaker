...
class TestOutboxRaceConditionFix:
    ...
    @pytest.mark.asyncio
    async def test_create_relationship_captures_outbox_id(self):
        """Test that create_relationship captures and uses outbox_id correctly."""

        # Mock all dependencies
        with patch('tools.relationship_tools.neo4j_service') as mock_neo4j, \
             patch('tools.relationship_tools.event_store') as mock_event_store, \
             patch('tools.relationship_tools.outbox') as mock_outbox, \
             patch('projections.neo4j_projection.Neo4jProjection') as mock_projection_class:
            ...
            mock_outbox.add_to_outbox.return_value = expected_outbox_id
            ...
            mock_outbox.mark_processed.assert_called_once_with(expected_outbox_id)

            # CRITICAL: Verify that get_pending was NOT called (old buggy behavior)
            mock_outbox.get_pending.assert_not_called()

    @pytest.mark.asyncio
    async def test_delete_relationship_captures_outbox_id(self):
        ...
        with patch('tools.relationship_tools.neo4j_service') as mock_neo4j, \
            ...
            mock_neo4j.execute_read.return_value = [{"relationship_id": "rel-123"}]

            # Setup event store
            mock_event_store.get_latest_version.return_value = 1
    ...
    @pytest.mark.asyncio
    async def test_concurrent_creates_use_correct_outbox_ids(self):
        """
        ...
        with patch('tools.relationship_tools.neo4j_service') as mock_neo4j, \
            ...
            mock_outbox.add_to_outbox.side_effect = outbox_ids
            mock_outbox.mark_processed.side_effect = mock_mark_processed
    ...
    @pytest.mark.asyncio
    async def test_projection_failure_does_not_call_mark_processed(self):
        ...
        with patch('tools.relationship_tools.neo4j_service') as mock_neo4j, \
            ...
            # Setup outbox
            mock_outbox.add_to_outbox.return_value = "outbox-fail"
...
class TestOutboxIdLogging:
    ...
    @pytest.mark.asyncio
    async def test_create_relationship_logs_outbox_id(self):
        """Test that the outbox_id is logged when added to outbox."""

        with patch('tools.relationship_tools.neo4j_service') as mock_neo4j, \
             patch('tools.relationship_tools.event_store'), \
             patch('tools.relationship_tools.outbox') as mock_outbox, \
             patch('projections.neo4j_projection.Neo4jProjection'), \
            ...
            mock_outbox.add_to_outbox.return_value = "outbox-log-test"
