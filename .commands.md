# 📚 Automaker Commands Guide

Quick reference for all available slash commands. Use these to get detailed context exactly when you need it.

---

## 🎯 Quick Reference

| Command            | Category        | Use When...                       |
| ------------------ | --------------- | --------------------------------- |
| `/automaker-arch`  | 🏗️ Architecture | Starting any feature work         |
| `/worktree`        | 🌳 Worktree     | Port issues, parallel development |
| `/new-feature`     | ✨ Features     | Building a new major feature      |
| `/env`             | ⚙️ Config       | Environment variable issues       |
| `/test-guide`      | 🧪 Testing      | Running or debugging tests        |
| `/ai-library-api`  | 📚 Knowledge    | Working on AI Library backend     |
| `/ai-library-ui`   | 🎨 Knowledge    | Working on AI Library frontend    |
| `/session-cleanup` | 🔄 Knowledge    | Session cleanup workflow          |
| `/sync`            | 🔀 Git          | Syncing worktree with main        |
| `/review`          | 👀 Review       | Code review with deep analysis    |
| `/commands-meta`   | 📝 Meta         | Creating/updating commands        |

---

## 🏗️ Architecture & Development

### `/automaker-arch`

**Full architecture reference** - server structure, frontend patterns, SYSTEMS feature, data storage, event system, execution flow.

```
Use when:
├── Starting work on any feature
├── Need to understand where code lives
├── Looking for service/route locations
└── Debugging execution flow
```

### `/new-feature`

**Feature-first architecture guide** - directory structure templates, type definitions, canonical references.

```
Use when:
├── Building a new major feature
├── Need directory structure template
├── Setting up frontend + backend + types
└── Following feature-first patterns
```

### `/env`

**Environment variables reference** - all config options with defaults and descriptions.

```
Use when:
├── Configuring the application
├── Debugging startup issues
├── Setting up new environment
└── Understanding port configuration
```

---

## 🌳 Worktree & Git

### `/worktree`

**Worktree port configuration** - allocation table, .env setup, simultaneous operation rules.

```
Port Allocation:
┌──────────────┬─────────┬─────────────┐
│ Location     │ UI Port │ Server Port │
├──────────────┼─────────┼─────────────┤
│ Main repo    │ 3007    │ 3008        │
│ feature-1    │ 3017    │ 3018        │
│ feature-2    │ 3027    │ 3028        │
└──────────────┴─────────┴─────────────┘

Use when:
├── Port already in use errors
├── Setting up new worktree
├── Running parallel development
└── Debugging worktree issues
```

### `/sync`

**Universal worktree sync** - synchronize branch with main, preserve all work, PR mode available.

```
Syntax:
├── /sync       → Current worktree
├── /sync 1     → feature-1 worktree
├── /sync 2     → feature-2 worktree
├── /sync:pr    → Create PR instead of merge
└── /sync 1:pr  → PR mode for feature-1
```

---

## 🧪 Testing

### `/test-guide`

**Complete testing guide** - E2E vs unit, commands, worktree testing, mock agent mode, debugging.

```
Quick Commands:
├── npm run test          → E2E (headless)
├── npm run test:headed   → E2E (visible)
├── npm run test:server   → Unit tests
└── npm run test:all      → Everything

Use when:
├── Running tests
├── Debugging test failures
├── Setting up CI
└── Understanding test structure
```

---

## 📚 Knowledge Library (AI Library)

### `/ai-library-api`

**Backend API reference** - REST endpoints, schemas, WebSocket events, session lifecycle.

```
Use when:
├── Working on Python backend
├── Adding/modifying endpoints
├── Understanding session workflow
└── Debugging API issues
```

### `/ai-library-ui`

**Frontend components reference** - React structure, hooks, state machine, component hierarchy.

```
Use when:
├── Working on React components
├── Modifying workflow UI
├── Understanding state management
└── Adding new UI features
```

### `/session-cleanup`

**Session cleanup workflow** - phases, state machine, transitions, WebSocket events.

```
Workflow:
┌─────────────────────────────────────────────────────────────┐
│  idle → file_staged → creating_session                      │
│    → cleanup_generating → cleanup_review                    │
│    → routing_generating → routing_review                    │
│    → ready_to_execute → executing → completed               │
└─────────────────────────────────────────────────────────────┘

Use when:
├── Working on cleanup feature
├── Debugging state transitions
├── Modifying workflow phases
└── Understanding WebSocket events
```

---

## 👀 Code Review

### `/review`

**Code review with deep analysis** - launches 5 deepdive agents + 5 deepcode agents for comprehensive review.

```
Use when:
├── Before merging to main
├── Comprehensive code analysis needed
├── Security/quality review
└── After major changes
```

### `/deepreview`

**Extended code review** - similar to /review with optional target branch.

---

## 🔧 Build & Validation

### `/validate-tests`

**Run and fix tests** - intelligent test fixing workflow.

### `/validate-build`

**Run and fix builds** - build validation workflow.

---

## 📝 Meta

### `/commands-meta`

**Command management guide** - creating new commands, updating existing ones, maintaining token efficiency.

```
Use when:
├── Creating a new command
├── Updating existing commands
├── Understanding command architecture
└── Maintaining token efficiency
```

---

## 🗺️ Decision Flowchart

```
What are you doing?
│
├─► Starting feature work?
│   └─► /automaker-arch
│
├─► Building new feature?
│   └─► /new-feature
│
├─► Port/worktree issues?
│   └─► /worktree
│
├─► Running tests?
│   └─► /test-guide
│
├─► Working on AI Library?
│   ├─► Backend? → /ai-library-api
│   ├─► Frontend? → /ai-library-ui
│   └─► Cleanup workflow? → /session-cleanup
│
├─► Syncing changes?
│   └─► /sync [1|2] [:pr]
│
├─► Code review?
│   └─► /review
│
├─► Environment config?
│   └─► /env
│
└─► Managing commands?
    └─► /commands-meta
```

---

## 💡 Tips

1. **Start lean**: CLAUDE.md is intentionally minimal (~85 lines). Load detailed context only when needed.

2. **Combine commands**: You can invoke multiple commands in one session if working across areas.

3. **Automaker agents**: These commands are for Claude Code sessions. Automaker's AI agents get context from `.automaker/context/` files automatically.

4. **Fork safety**: Always use `/sync` for git operations - it includes safety checks for the fork workflow.

5. **When in doubt**: `/automaker-arch` gives you the broadest context for general development work.
