...
export class ClaudeUsageService {
  ...
  private executeClaudeUsageCommandPty(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      ptyProcess.onData((data: string) => {
        ...
        // Strip ANSI codes for easier matching - use the same comprehensive pattern
        ...
        if (
          ...
          setTimeout(() => {
            if (!settled && ptyProcess && !ptyProcess.killed) {
              ...
              // Fallback: if ESC doesn't exit (Linux), kill the process after 2s
              // Windows doesn't support Unix signals, so call kill() without args
              setTimeout(() => {
                if (!settled && ptyProcess && !ptyProcess.killed) {
                  ptyProcess.kill();
                }
  ...
  private parseResetTime(text: string, type: string): string {
    ...
    // Note: \s* allows for optional space after "resets" to handle cases where
    // ANSI stripping may remove whitespace (e.g., "Resets6pm" from terminal output)
