...
function handleFetchError(error: unknown): never {
  ...
  throw new KnowledgeLibraryError(error instanceof Error ? error.message : 'Unknown error', {
...
export const knowledgeLibraryApi = {
  ...
  async decideCleanupItem(
    ...
    return request<KLSuccessResponse>(`/api/sessions/${sessionId}/cleanup/decide/${blockId}`, {
      method: 'POST',
      body: data,
    });
  },
  ...
  async approveCleanupPlan(sessionId: string): Promise<KLSuccessResponse> {
    return request<KLSuccessResponse>(`/api/sessions/${sessionId}/cleanup/approve`, {
      method: 'POST',
    });
  },
  ...
  async generateRoutingPlan(
    ...
    return request<KLRoutingPlanResponse>(`/api/sessions/${sessionId}/plan/generate`, {
      method: 'POST',
  ...
  async selectDestination(
    ...
    return request<KLSuccessResponse>(`/api/sessions/${sessionId}/plan/select/${blockId}`, {
      method: 'POST',
      body: data,
    });
  },
  ...
  async rejectBlock(sessionId: string, blockId: string): Promise<KLSuccessResponse> {
    return request<KLSuccessResponse>(`/api/sessions/${sessionId}/plan/reject-block/${blockId}`, {
      method: 'POST',
    });
  },
