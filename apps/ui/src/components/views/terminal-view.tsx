...
export function TerminalView() {
  ...
  // cwd: optional working directory for the terminal (defaults to currentProject.path)
  const createTerminal = async (
    ...
    try {
      ...
      // Use provided cwd, then pendingTerminalCwd, then currentProject.path
      const terminalCwd =
        cwd || terminalState.pendingTerminalCwd || currentProject?.path || undefined;

      const response = await apiFetch('/api/terminal/sessions', 'POST', {
        headers,
        body: { cwd: terminalCwd, cols: 80, rows: 24 },
      });
      const data = await response.json();
