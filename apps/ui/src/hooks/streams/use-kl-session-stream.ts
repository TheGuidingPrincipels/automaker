...
import type { KLStreamEvent, KLStreamCommand, KLStreamCommandRequest } from '@automaker/types';
...
export function useKLSessionStream(
  ...
  const eventToTranscript = useCallback((event: KLStreamEvent): KLTranscriptEntry | null => {
  ...
  const handleEvent = useCallback(
    (event: KLStreamEvent) => {
      ...
      if (questionPayload) {
        setPendingQuestions((prev) => [...prev, questionPayload]);
      }
    },
    [eventToTranscript]
  );
